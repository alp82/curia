...
1. On the **Tailscale** card, read the identity the panel names, `You opened Curia as <login> through Tailscale`. It's the login Tailscale stamped on your request.
2. Keep the machine name `curia.sh` or enter the node's current name, which the panel shows beside the field. Curia never renames the node. To rename it, run `sudo tailscale set --hostname <name>` on the host.
