...
| `this node is named <x>, not <y>` (not a refusal) | The node was already logged in under another name. | Nothing; the existing name wins, and the install continues with it. To use your name, run `sudo tailscale set --hostname <name>` on the host and rerun, or reinstall with `--name`. |
...
| Discord: `The commands registered in <server> differ from curia's` | An update changed a command, or another tool replaced the registered ones. | Select **Register commands** in the panel. If Discord refuses that, add the bot again with the invite link, which asks for the `applications.commands` scope. |
...
| The bot stopped answering in the channel | The bridge lost its token or the service is down. | Run `curia doctor`. If the Discord card reads connected and the bridge isn't running, select **Try again** on the card, which starts the bridge, and read the service log. |
