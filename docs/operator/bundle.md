...
The release workflow (`.github/workflows/release.yml`) runs when Release Please drafts the release. It pushes each image, records the digest the registry returned, attests the build provenance of that digest, renders the bundle against the four digests, writes the release manifest from the bundle's checksum and the digests, runs the bundle and manifest tests against the rendered files, attaches the bundle, its SHA-256 checksum (`curia-bundle-<version>.tar.gz.sha256`), the digest set, the manifest, and the bootstrap to the draft release, publishes the release, and publishes the package last. The order is in [Releases, the stable-release index, and version selection](releases.md).
