...
|---|---|---|
| Five release images | `ghcr.io/alp82/curia-daemon`, `curia-tmux`, `curia-dashboard`, `curia-overseer`, `curia-agent` | The Curia service, the tmux runtime (which also serves the attach surface), the Curia app, the overseer, and the agent image that every agent container starts from. Each is built once per release from the `release` stage of its Dockerfile and identified by its digest. |
| The Compose bundle | `curia-bundle-<version>.tar.gz` on the GitHub release, unpacked to `versions/<version>/bundle/` in the installation root | One `compose.yaml` that names the four service images by exact digest. The agent image runs no service, so the bundle doesn't name it. |
...
| The release manifest | `curia-manifest-<version>.json` on the GitHub release, and `manifest.json` inside the `@curia-sh/cli` package | The one file that binds the package version, the bundle checksum, the five digests, and the source commit into one release. See [The release manifest and release verification](release-manifest.md). |
| The bootstrap | `curia-install.sh` on the GitHub release, under the same name on every release | The script that installs or purges Curia on a host with no Node.js. It selects the version from the stable-release index and pins nothing but its own version. See [The bootstrap](bootstrap.md). |

The release workflow (`.github/workflows/release.yml`) runs when Release Please drafts the release. It pushes each image, records the digest the registry returned, attests the build provenance of that digest, renders the bundle against the four service digests, writes the release manifest from the bundle's checksum and all five digests, runs the bundle and manifest tests against the rendered files, attaches the bundle, its SHA-256 checksum (`curia-bundle-<version>.tar.gz.sha256`), the digest set, the manifest, and the bootstrap to the draft release, publishes the release, and publishes the package last. The order is in [Releases, the stable-release index, and version selection](releases.md).
...
Each service image carries the checkout at `/opt/curia`: the daemon's code and pinned dependency tree, the lifecycle interface's source, the shipped `curia.yaml` and `routing.yaml`, and, in the service image, the vendored skills and the agent image recipe. The files are root-owned and read-only to the user that runs the container, so an installed version can't drift. The images contain no operator path, no credential, and no state. What a container may see of the installation root is in [Secrets, mounts, and what survives](secrets.md).

## The agent image

The agent image, `ghcr.io/alp82/curia-agent`, is the fifth release image. It carries the harness command-line tools at the versions `curia.yaml` pins (Claude Code, Codex, `gh`, `ttyd`, and the browser libraries Playwright needs), and the service starts one container from it per agent and per model sign-in. It's built once per release from `deploy/agent/Dockerfile` with the same pins, attested, and bound in the release manifest like the other four.

The bundle doesn't name it, because no Compose service runs it. `curia install` and `curia update` pull it by digest beside the bundle's images, and the service reads its digest reference from the installed release manifest at `versions/<version>/cli/manifest.json`. The service never builds an image on your host, and the **Model provider** card never waits for a build. When a `curia update` switches releases, the next agent starts from the target release's agent image; a running agent keeps the image it started on.

The image is built for user ID 1000 and runs as your numeric user ID through `--user`, so a fresh installation runs as you. `curia purge` removes it with the other release images when no container uses it.
...
- **Labelled resources.** Every container, the Compose network, and the tmux socket volume carry the label `sh.curia.installation=<installation ID>`, the ID from `state/installation.json`. The service puts the same label on every agent container and agent cache volume it creates. `curia uninstall` and `curia purge` remove Docker resources by that label and never by a name.
