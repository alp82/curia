...
| `--name <machine-name>` | The name this host joins your tailnet under when it isn't logged in yet. Default: `curia`. A MagicDNS label: lowercase letters, digits, and hyphens, up to 63 characters. The script hands it to `curia install`, whose `tailnet` step does the login; see [The tailnet step](install.md#the-tailnet-step). Anything else is a usage error (exit code `2`) before any download, and a purge takes no name. |
...
2. **Selects the version.** It downloads the stable-release index and reads the stable release from it, or takes the version you passed. At this point the index is a hint: the signature is verified in step 5, by code that carries the key. A purge that can run the installed interface never reaches this step, and never reads the index; see [Purge mode](#purge-mode).
...
A purge removes an installation and activates nothing, so it needs no release, and the script takes the first of three sources that answers:

1. **The installed interface.** When the root holds a complete active version, the script runs `versions/<active>/cli` on the runtime beside it at `versions/<active>/node`, the pair the launcher runs, so a missing launcher doesn't matter. That version proves itself first, from the artifacts retained beside it, through the door `curia doctor` uses, without the four checks that need an origin: the registry's integrity record and provenance, the release's copy of the manifest, and the image attestations. Nothing is downloaded, and the index isn't read. A version that doesn't verify is refused, and the refusal names `--version` as the way to purge with a downloaded interface.
