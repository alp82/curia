...
6. **Hands off.** It runs `curia install` (or `curia purge`) from the stage, on the staged runtime, with `CURIA_ROOT` set to the root and, for an installation, `CURIA_STAGE` set to the stage. The lifecycle interface owns the root, the record, the launcher, and every confirmation from there. The script exits with the interface's own exit code.
