...
2. Install the target as in [2. Install Curia](guide/02-install-curia.md), with the target release: `curl -fsSLO https://github.com/alp82/curia/releases/latest/download/curia-install.sh && bash curia-install.sh --version 0.14.0`. Release 0.14.0 requires a downloaded file and refuses piped execution. Don't connect any service on the Setup screen: the cutover brings the credentials, and a Discord card connected here would put the bot token in a second running service.
...
3. Open the Setup screen and let every card verify fresh, as in [3. Connect services](guide/03-connect-services.md). Nothing is re-registered:
   - **GitHub** reauthorizes the existing App as the operator, then proves its installations cover the watched repositories on a fresh token. If the migrated App lacks OAuth credentials, use **Connect existing App** as described in [integration setup](integration-setup.md#connect-an-existing-app-after-migration). The recovery form requires a release containing the existing-App authorization flow; 0.14.0 does not provide it. Otherwise select **Authorize GitHub**. The host's `gh auth login` does not replace this step. Keep the existing App and its installations.
