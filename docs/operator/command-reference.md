...
| `versions/` | Installed versions: the pinned runtime, the lifecycle interface, the release manifest, the Compose bundle, and the retained artifacts of each. After an update, the active release and the previous one, which is the rollback release. See [Release images and the Compose bundle](bundle.md), [The release manifest and release verification](release-manifest.md), and [What is kept for a rollback](update.md#what-is-kept-for-a-rollback). | Replaceable |
...
| `curia rollback` | Switches back to the one retained previous release, through the same checks as the switch. | Later release |
