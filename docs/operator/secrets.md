...
The allowed operator lives in `state/tailscale.json`: the Tailscale login the [Tailscale card](integration-setup.md#connect-tailscale) recorded when you confirmed it, when it was confirmed, the node's machine name as the tailnet reports it, and the Serve routes Curia created. Under an installation root that login is the whole identity allowlist for the app and every published surface. The `identity.allow` list in `curia.yaml` admits nobody there. Until the file names an operator, the app admits the first tailnet identity that opens it, and only to **Setup**.
...
| `curia purge` | Nothing local. The GitHub App, the Discord bot, the Tailscale node, and the model-provider logins stay valid where they were issued. | The entire root, every Curia-labelled Docker resource, the release images that nothing on the host uses, Curia's Serve routes, and the launcher. See [Purge and external cleanup](purge.md). |
