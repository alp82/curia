...
- **An owner the app is not installed on has no fallback here.** The agents kept `CURIA_AGENT_GH_TOKEN_*` for that case (#389). The overseer keeps nothing: it reads public repositories only, and it names the owner in the chat once per turn. That was the operator's decision on this ticket, and the reason is that a per-owner PAT kept alive forever is the cost ADR-0018 exists to remove. **The agents came the same way on [#466](https://github.com/alp82/curia/issues/466)**, which retired that key. They keep less than the overseer does: a dispatch to such an owner is refused, because an agent that can only read public repositories cannot resolve a ticket.
