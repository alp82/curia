...
## The Open Graph card: `bin/og-card.py`

The link preview card is `docs/og.png`, 1200x630, generated by `bin/og-card.py` and committed.
Re-run it whenever the card's words change:

```
python3 bin/og-card.py
```

**Why a script and not a checked-in drawing.** The card repeats the page's own words and palette, so
it goes stale the same way the page does. A script that reads like the page is cheaper to re-run
than a picture nobody can edit.
...
proves what it can before it writes: a self-test asserts the letter `H` sits above the baseline at
about cap height and covers about half its own box, which catches a flipped or mis-scaled transform,
and the build refuses to write a card with too few inked pixels. The operator's eyes are still the
gate — the card ships through a review gate like everything else.
