...
## The image assets: `bin/og-card.py`

The repository stores two generated image assets. The link preview card is `docs/og.png`, 1200x630.
The favicon is `docs/favicon.png`, 192x192. Re-run the script when either image changes:

```
python3 bin/og-card.py
```

**Why a script and not a checked-in drawing.** The images use the page palette, and the card repeats
the page words. A script keeps those choices visible and makes both images reproducible.

**Why pure Python.** The box has no image library, no `sharp` and no browser, and an agent may not
install one. The script is Python's standard library only: a TrueType reader, a scanline rasterizer
and a PNG writer, over the Liberation fonts already on the box. It is about 0.7 seconds.

**How it checks itself.** An agent never judges rendered output by looking at it, so the script
proves what it can before it writes. One self-test checks the card glyph transform and rasterizer.
Another checks the favicon glyph position and ink count. The build also refuses to write a card with
too few inked pixels. The operator gives the final approval for both images.

**SVG was not an option.** Facebook, X, Discord, Slack and LinkedIn do not render an SVG
`og:image`. The card has to be a raster.
