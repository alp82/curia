...
- Two questions stay open, and each earns its own grilling ticket on the build map. The first is the hosting shape: a second sidecar beside the dashboard, or an agent-shaped tmux pane. **Answered** by [ADR-0015](0015-the-overseer-is-a-service.md) on [#310](https://github.com/alp82/curia/issues/310), which took the service and added one container per thread as a third candidate before ruling it out. The second is the detail behind the per-thread conversation key.
