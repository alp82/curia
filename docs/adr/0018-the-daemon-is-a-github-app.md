...
- **The private key is Curia's one durable secret value.** The daemon copy sits at `daemon/.curia-app.pem`, mode 0600, uncommitted, named by `CURIA_GH_APP_KEY_FILE` beside `CURIA_GH_APP_ID` in `daemon/.env.daemon`. The key is a file and never a value in the daemon env file: a PEM carries newlines, Docker reads a newline in an env file as a second variable, and a path keeps the key out of every child process environment the daemon spawns. The `alp82/curia` Actions secret `CURIA_RELEASE_APP_PRIVATE_KEY` holds a second copy for the release workflow. GitHub-hosted runners can't reach the daemon's minting path, and this copy avoids adding a PAT.

- **The daemon mints for every runtime holder.** `daemon/src/githubapp.mjs` signs the app JWT with `node:crypto`, reads the installation list, and mints installation tokens. No container asks a daemon endpoint for a token: a shell that can mint is the capability [ADR-0014](0014-the-overseer-in-its-own-container.md) removed. The release workflow is the only exception. It mints a one-hour token inside GitHub Actions and passes the token directly to Release Please.

- **One key, three permission sets.** A minted installation token may scope down from what the installation grants. Agents get `contents:write, issues:write, pull_requests:write, metadata:read`. The overseer gets [#313](https://github.com/alp82/curia/issues/313)'s read set unchanged: `contents, issues, pull_requests, statuses, metadata`, all read. The release workflow gets `contents:write, issues:write, pull_requests:write` for `alp82/curia` only. ADR-0014's runtime boundary still holds because no agent or overseer container receives the private key.
...
- **The release copy widens the trusted store.** Repository administrators and workflows on the trusted default branch can use the Actions copy. Pull-request workflows don't receive it. Curia's app still has no `workflows` permission, so agent-authored pushes can't change the workflows that hold the copy.
