...
- **Its config dir is `<workspace_root>/cfg/curia-overseer`**, mounted at that same path inside the container. The name is the session name the timeline already serves. It sits beside the agent config dirs, so the daemon's `data/` stays out of a second container. The Chat screen reads the transcript from that directory, so the path must be identical on both sides.
...
- **Nothing expires a conversation.** A Discord thread starts empty, because every top-level message opens a new one. The console chat is one key forever, and it only grows. [#311](https://github.com/alp82/curia/issues/311) owns that question.
- **A conversation thread must stay single-use.** Work dispatched from a conversation takes over that thread and is renamed on purpose. It does not open a second thread beside it. Charting a map through the overseer opens two today, which the operator reported at this ticket's review gate. [#326](https://github.com/alp82/curia/issues/326) owns the fix, and [#311](https://github.com/alp82/curia/issues/311) decides the conversation key that fix must agree with.
