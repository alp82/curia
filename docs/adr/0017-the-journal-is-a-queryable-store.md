...
- **The existing file migrates, and it stays on disk.** **Settled by [#323](https://github.com/alp82/curia/issues/323)**: the daemon converts at first boot. It finds `events.db` absent, reads the journal file whole, inserts every line in one transaction, and renames the database into place after the row count matches the line count. The measured cost is 298 ms for 4,282 events. The file is not renamed and not deleted, because the self-deploy rollback has no human in it and the previous daemon looks for that exact path. A follow-up ticket deletes it once the journal is checked on the box. [The daemon README](../../daemon/README.md#the-migration-to-the-database) holds the rollback recipe.
