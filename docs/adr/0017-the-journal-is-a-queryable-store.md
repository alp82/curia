...
- **Retention becomes possible and stays undecided.** The text file never rotated. A row deletes. Nothing here rules on what curia keeps. One constraint is already known: the boot rebuild runs over the whole history, so a deleted row changes the reduction. `consoleSpent` holds every console conversation key ever minted and must never shrink ([#333](https://github.com/alp82/curia/issues/333)). Delete the oldest `console_conversation_opened` rows and a spent number can be minted again, which wakes a deleted conversation's memory.
...
- **The boot rebuild survives, and it reads the journal.** `EscalationStore._replay` reads the whole file once per process today and fills every [#289](https://github.com/alp82/curia/issues/289) reduction from that one pass. [#322](https://github.com/alp82/curia/issues/322) ruled that the rebuild stays. Three reductions keep rows verbatim and could be queries, and every other field folds many rows into an object no row holds. The reducer must also run on every append, so a query at boot would state each rule twice. The rebuild reads `select id, body from events where id > ? order by id limit 1000`, page by page. It orders by `id` and never by `ts`, because stamps tie. It reads `body`, so it is the last reader that runs `normalizeEvent`. `EscalationStore._replay` becomes `Reduction#rebuild`, because "replay" already names sending a killed turn's message again.
...
- **The word "store" now names two things.** `EscalationStore` lives in `daemon/src/store.mjs` and holds the in-memory reductions. The durable artifact above is also a store. Curia writes one name for one thing, so the build picks a second name before it writes code. **Settled by [#358](https://github.com/alp82/curia/issues/358)**: "store" names nothing in curia. The durable record keeps the name **journal**, whatever medium holds it, so the `node:sqlite` database IS the journal. The in-memory state is the **reduction**, and `EscalationStore` becomes `Reduction` in `daemon/src/reduction.mjs` when the build renames it. `CONTEXT.md` holds the vocabulary.
