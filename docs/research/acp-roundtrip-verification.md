...
- Spike client + MCP ask-server + SDK isolation test: [`prototypes/acp-roundtrip/`](../../prototypes/acp-roundtrip/) (`spike.mjs`, `mcp-ask-server.mjs`, `sdk-elicit-test.mjs`; scenarios: `basic` / `permission` / `elicit` / `elicit-free` / `askuser` / `image` / `steer` / `tools`). Wire logs land in `logs/*.ndjson` (gitignored).
