...
The script connects over ssh, pulls `main`, and runs `docker compose up -d --build daemon dashboard`. The daemon container installs its npm dependencies at start, so a dependency change needs no extra step. The script prints the two services' state on success. Override the target with `CURIA_DEPLOY_HOST`.
