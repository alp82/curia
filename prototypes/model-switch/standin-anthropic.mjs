// PROTOTYPE (#561) — stand-in Anthropic API for the claude model-switch probe.
// Serves POST /v1/messages (streaming SSE) and logs EVERY request — method,
// path, headers, body — to <dir>/req-<n>.json, so the model a turn states is
// read off the wire and never off a pane rendering.
//
// One addition over the #559 stand-in: a COLD model. Any request whose model
// id contains "cold" is refused with the Anthropic error shape, so the probe
// can watch how the pane renders a model the account refuses.
  ...
  if (req.method === 'POST' && req.url.includes('/v1/messages') && !req.url.includes('count_tokens')) {
    ...
    if (model.includes('cold')) {
      res.writeHead(403, { 'content-type': 'application/json' }).end(JSON.stringify({
        type: 'error',
        error: { type: 'permission_error', message: `You do not have access to the model ${model}.` },
      }))
      return
    }
    if (!body?.stream) {
      // The /model validation probe asks non-streaming: answer the plain shape.
      res.writeHead(200, { 'content-type': 'application/json' }).end(JSON.stringify({
        id: `msg_${n}`, type: 'message', role: 'assistant', model,
        content: [{ type: 'text', text: `ok (req ${n}, model ${model})` }],
    ...
    ev('content_block_delta', { index: 0, delta: { type: 'text_delta', text: `ok (req ${n}, model ${model})` } })
