// PROTOTYPE (#561) — throwaway driver for the model-switch probes.
...
if (cmd === 'new') {
  ...
  // put: literal text WITHOUT Enter — for composer-queue probes.
  const cp = await import('node:child_process')
  cp.execFileSync('tmux', ['send-keys', '-t', `=${name}:`, '-l', rest.join(' ')])
} else {
