...
describe('a node that is logged in', () => {
  test('is reported with its name and MagicDNS address, and nothing is changed', async () => {
    ...
    assert.match(r.out, /^the node name is curia, chosen with --name\n/, 'the chosen name is the first thing the step says')
  ...
  test('under another name than --name says so as a fact and continues with the actual name', async () => {
    ...
    assert.match(r.out, /^the node name is curia, chosen with --name\n/)
    assert.match(r.out, /this node is named alp-workstation, not curia\. The existing name wins: Curia never renames a node, and the installation continues with alp-workstation\./)
