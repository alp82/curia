...
`CURIA_OVERSEER_GH_TOKEN_<OWNER>` in `daemon/.env.overseer` was the PAT this replaced. The key is retired, and boot names one that is still in the file. What is left in that file is a first-boot **seed** of the model credential, read once and then named the same way ([#648](https://github.com/alp82/curia/issues/648)).
