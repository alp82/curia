...
**Overview**:
The daemon's one loopback read of itself, `GET /overview`. It joins every section the dashboard draws. Sections include agents, escalations, health, usage, warnings, journal events, settings, and deploy status. The Overview also carries the frontier snapshot and a map snapshot. Each map snapshot includes history, agents, frontier facts, blockers, fog, counts, and its latest event stamp. Journal events and each Overview poll invalidate the map snapshot.
