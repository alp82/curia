...
`CURIA_OVERSEER_GH_TOKEN_<OWNER>` in `daemon/.env.overseer` was the PAT this replaced. The key is retired, and boot names one that is still in the file. What is left in that file is the model credential.
...
The tmux session a device login runs in, named `curia-auth-<consumer>`. On the codex lane it runs `codex login --device-auth` in the agent image, which prints a link and a one-time code that lives fifteen minutes and needs nothing pasted back. The operator drives it from a phone through the ttyd attach surface, so the recovery from a dead credential has no ssh in it. One session per consumer, enforced by the fixed name. Completion is detected by the credential file appearing in the scratch config dir, and there is a 30-minute timeout. Every outcome is journaled, because a re-authentication that silently vanished is the same class of bug as the credential that silently vanished. **No sweep ever walks one**: not the liveness sweep, not the stall sweep, not reconcile, not the credential sweep, not the container sweep. The dashboard scrapes the link and the code onto a card, and degrades to "open the terminal" when the scrape misses. Discord gets the alarm and a link, and never the code. See [#642](https://github.com/alp82/curia/issues/642) and the live checks for [#644](docs/live-checks/644-credential-swap-heals.md) and [#642](docs/live-checks/642-codex-reauth.md).
