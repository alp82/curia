...
**Rollback release**:
The one release under `versions/` beside the active one after a switch: the release that was active before it ([#884](https://github.com/alp82/curia/issues/884), implementing [#854](https://github.com/alp82/curia/issues/854)). The switch removes every other version, so Curia keeps one step of history and no more. `curia rollback` ([#885](https://github.com/alp82/curia/issues/885)) switches back to it through the same door. A staged target that failed its switch is not one: it stays for the rerun and is removed by the next successful switch.
