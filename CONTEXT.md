...
**Ordinary uninstall**:
Removal of Curia's runnable system from one host while preserving operator configuration, secrets, durable state, and resumable work in the installation root. It reports the preserved root and the paths to reinstall or purge, each naming the version that was uninstalled ([#891](https://github.com/alp82/curia/issues/891)), so the printed commands work before the first stable release is promoted. External service resources remain untouched. `curia uninstall` ([#886](https://github.com/alp82/curia/issues/886)) is four named steps, `preflight`, `docker`, `routes`, and `files`: under the lifecycle lock it removes every container, network, and volume that carries the installation label, turns off the Serve routes recorded in `state/tailscale.json`, empties `versions/`, `cache/`, and `run/`, and removes the launcher; the container images stay for the confirmed purge. Every step reads before it removes, so a rerun finishes a partial cleanup. See [docs/operator/uninstall.md](docs/operator/uninstall.md).
...
**Full loop**:
The one end-to-end pass through the system: frontier, dispatch, escalation and answer, review, merge, resolution, map update. The name of the eight-leg cycle itself; a **Test run** runs one Full loop per ticket.
...
**Rehearsal**:
A scripted live run of the full loop on real repos. It proves every leg in one unbroken pass. The tickets a rehearsal spends carry the `rehearsal` label; since [#891](https://github.com/alp82/curia/issues/891) the **Test run** creates them itself.

**Test run**:
The installation's acceptance ([#882](https://github.com/alp82/curia/issues/882), implementing [#857](https://github.com/alp82/curia/issues/857); reshaped by the [#891](https://github.com/alp82/curia/issues/891) rehearsal): `daemon/src/fullloop.mjs`, with the map's text in `daemon/src/testrunmap.mjs`. **Start Test run** takes this read's **Full-loop gate** and creates a tiny wayfinder map in the covered repository, "Test run <date>", with two child tickets, **Add a line to the README** and **Remove the Test run line from the README**, the second blocked by the first through GitHub's native dependency, both marked `rehearsal` and typed `wayfinder:test-run`, the routing row that names the cheapest model of a provider at its lowest effort for the agent and for the cross-check alike (the owner's decision on the [#891](https://github.com/alp82/curia/issues/891) rehearsal, which had spent `gpt-5.6-sol` at high effort on a README line); each write is journalled as it lands, so **Try again** resumes the same map, and a second Test run creates a new one. The status names the model and the effort the run dispatches on. The run finds each ticket through the dispatcher's own frontier read and dispatches it through the dispatcher's own `start`, ticket 1 through the eight legs of the Full loop and then ticket 2 through the same eight, and finishes on the ninth leg, **Map closed**, which is the map lifecycle's own close after the operator's empty-map verdict. Every leg is judged from the journal rows the daemon already writes while the agents work, counted only after the run's spawn, only for the ticket's session, and only in order; a row from an earlier dispatch, another ticket, the reviewer, or out of order counts for nothing, and neither does the run's own completion row. The status names the map, both tickets, and which is in flight (1 of 2, 2 of 2). A failure names the leg, one cause, and one action, keeps the completed legs, the map, the tickets, and the connected integrations, and **Try again** reruns the failed leg. Nothing is stored but the journal rows, so `curia doctor` keeps reading the gate. See [Start Test run](docs/operator/integration-setup.md#start-test-run). The run carries what it waits for: the last open question addressed to the ticket in flight or to the map, with the leg that runs, the time since it opened, and `message`, the escalation as the bridge composed it (`daemon/src/escalationembed.mjs`, the one composer the bridge sends from), which the run panel draws as a Discord-style embed with the page's answer controls under it and answers through the daemon's own `/answer` route, the one the Discord bridge uses, so the journal sees one `esc_answer` whichever surface answered first; and the live sessions, the agent's and the overseer's in a turn, each with the app's terminal link. Leaving Setup never cancels or detaches the run: Home shows it with its leg and returns to the panel. There is no **Close setup**.
_Avoid_: Full-loop run, Run Full loop, rehearsal ticket (the run makes its own).
...
What the daemon reaches GitHub with for its own work: the frontier reads, the claim, the clone, the pull request and the branch push. One minted write token per resource owner, put in the environment of the one `gh` or `git` child that needs it and never in the daemon's own. The repo the call names is what picks the owner, so nothing downstream holds a minter. It is `GH_TOKEN` rather than a `gh` config dir, because the daemon spawns its own children and has no container mount to serve. The settings repository picker lists repositories covered by the App installations through `daemon/src/watchablerepos.mjs`. Only source deployments without an App retain `viewerLogin()` and `gh api user/repos` as a host-login fallback. Packaged installations never use that fallback. Successful reads are cached for ten minutes; explicit refresh bypasses the cache, transient failures retry twice, and failed reads retain the last successful list without caching the failure. An owner the app is not installed on falls back to the host login, loudly. Lives in `daemon/src/daemongh.mjs`. See [#390](https://github.com/alp82/curia/issues/390).
_Avoid_: daemon token, host login (that one is the operator's own `gh`, which keeps dev sessions, the deploy sibling and the gate approval on the source box).

**Operator authorization**:
The operator's own GitHub credential on a packaged installation, and what the review gate's approval is posted with there. GitHub grants it when the operator installs the App: the manifest sets `request_oauth_on_install` and `setup_on_update` and names a callback, GitHub sends a one-use code there, and the daemon exchanges it with the App's client id and secret (stored beside the key in `secrets/github-app.json`) for a user token. The token, its refresh token, their expiry, and the login live in `secrets/github-operator.json`, owner-only, and the daemon refreshes the token inside a ten-minute margin before it hands it to the one `gh` child that posts the approval. The GitHub card proves it on `GET /user` and reports the login; a missing or refused one fails the card at the install step, and the cure is to reinstall the App, which repeats the authorization. The operator never runs `gh auth login`. Lives in `daemon/src/githuboperator.mjs`, routed by `daemon/src/daemongh.mjs`. Decided at [ADR-0031](docs/adr/0031-the-operator-authorizes-curia-when-the-app-is-installed.md) on [#891](https://github.com/alp82/curia/issues/891).
_Avoid_: host login (the source box's `~/.config/gh`, which a container has none of), user token (GitHub's word for the value, and the value is never named outside the file), OAuth app (it is the GitHub App's own user authorization, and no second App exists).
...
**Pairing table**:
The `review:` section of `routing.yaml`, keyed by the builder's provider. An anthropic builder gets `gpt`, an openai builder gets `opus`. A row keyed by a ticket type (`review.test-run`) holds that type's own pairing by builder provider and beats the provider rows for that type. A `review-model:<name>` label on the ticket beats both.
...
- **Secret files** (`<root>/secrets/`): in an installed Curia, the five long-lived credentials as one owner-only file each — `discord-bot-token`, `github-app.json`, `github-operator.json`, `anthropic.json`, `codex-auth.json` — read and written through `cli/src/secrets.mjs` and mounted into the service alone ([#867](https://github.com/alp82/curia/issues/867)). The service refuses to boot with a credential in its environment. `cfg.paths` (`daemon/src/paths.mjs`, from `cli/src/layout.mjs`) is where every process learns the paths: `state/` for the journal, `work/` as the workspace root, `cache/home` as `HOME`, `cache/overseer-repos`, `run/overseer-tokens`. The operator's page is [Secrets, mounts, and what survives](docs/operator/secrets.md).
