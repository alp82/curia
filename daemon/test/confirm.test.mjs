// Button confirms at the store layer (#94): the `confirm` kind's own
...
describe('confirm records (#94)', () => {
  test('open persists the action and the issuing thread, and both survive a replay', () => {
    ...
    const { record } = openConfirm(store, [target('curia-42', 'curia-42@1')])
    ...
    const replayed = new EscalationStore(dir).get(record.id)
  ...
  test('a newer confirm sharing a target instance supersedes the older, whatever the wording', () => {
    ...
    assert.equal(store.get(older.id).successor, newer.id)
    // an answer to the dead id routes to the live confirm (reused chain)
    const r = store.answer(older.id, { answer: 'approve', by: 'alp', via: 'button' })
  ...
  test('a confirm on a DIFFERENT instance of the same session does not supersede', () => {
    const store = new EscalationStore(dir)
    const { record: older } = openConfirm(store, [target('curia-42', 'curia-42@1')])
    const { superseded } = openConfirm(store, [target('curia-42', 'curia-42@2')])
    assert.equal(superseded, null)
    assert.equal(store.get(older.id).status, 'open')
  })
  ...
  test('lapse closes the record with its reason, refuses non-open records, and survives a replay', () => {
    const store = new EscalationStore(dir)
    const { record } = openConfirm(store, [target('curia-42', 'curia-42@1')])
    const r = store.lapse(record.id, '`curia-42` finished')
    assert.equal(r.ok, true)
    assert.equal(store.get(record.id).status, 'lapsed')
    assert.equal(store.get(record.id).lapse_reason, '`curia-42` finished')
    assert.equal(store.lapse(record.id, 'again').ok, false, 'a closed record never lapses twice')
    assert.equal(store.answer(record.id, { answer: 'approve', by: 'alp', via: 'button' }).ok, false,
      'a lapsed confirm is not answerable — the buttons are dead')

    assert.equal(new EscalationStore(dir).get(record.id).status, 'lapsed')
  })
})
...
describe('overseer notes (#94)', () => {
  test('notes queue per thread, drain once, and the drain survives a replay', () => {
    const store = new EscalationStore(dir)
    store.addOverseerNote('thread-9', 'confirm esc-1 approved — cancelled curia-42')
    store.addOverseerNote('thread-9', 'confirm esc-2 lapsed — `curia-43` finished; nothing was executed')
    store.addOverseerNote('thread-other', 'unrelated')

    assert.deepEqual(store.takeOverseerNotes('thread-9'), [
      'confirm esc-1 approved — cancelled curia-42',
      'confirm esc-2 lapsed — `curia-43` finished; nothing was executed',
    ])
    assert.deepEqual(store.takeOverseerNotes('thread-9'), [], 'drained notes never replay into a later turn')
  ...
  test('an empty take writes no drain event', () => {
    const store = new EscalationStore(dir)
    assert.deepEqual(store.takeOverseerNotes('thread-9'), [])
    assert.equal(fs.existsSync(path.join(dir, 'events.jsonl')), false, 'no notes ⇒ no journal traffic')
  })
})
