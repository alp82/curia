...
describe('the verb catalogue serves both transports (#314)', () => {
  // #692 (ADR-0022) made the map tool two tools, so the catalogue is ten.
  ...
  test('one catalogue, ten verbs, in one order', () => {
    assert.deepEqual(VERB_TOOLS, ['tickets', 'next', 'status', 'skill', 'start', 'map_update', 'map_new', 'cancel', 'resume', 'attach'])
  ...
  test('the HTTP transport publishes the same ten, with the same schemas', async () => {
    const posted = []
