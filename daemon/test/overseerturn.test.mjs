...
describe('the verb catalogue serves both transports (#314)', () => {
  // #692 (ADR-0022) made the map tool two tools. Chat ticket creation adds the
  // eleventh without changing the map grammar.
  ...
  test('one catalogue, eleven verbs, in one order', () => {
    assert.deepEqual(VERB_TOOLS, ['tickets', 'next', 'status', 'skill', 'start', 'ticket_new', 'map_update', 'map_new', 'cancel', 'resume', 'attach'])
  ...
  test('the HTTP transport publishes the same eleven, with the same schemas', async () => {
    const posted = []
