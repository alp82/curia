...
describe('the settings screen (#265)', () => {
  ...
  test('Settings starts from a drill-in list and holds dirty edits in one save dock', () => {
    page.UI.set.section = null
    let html = page.screenSettings(payload())
    assert.match(html, /class="settings-home"/)
    assert.match(text(html), /Routing.*Projects.*Dispatch.*Connections.*Maintenance/)
    assert.ok(!html.includes('class="save-dock"'))

    page.setSection('routing')
    page.setDispatchField('max_concurrent', '4')
    html = page.screenSettings(payload())
    assert.match(text(html), /‹ Settings.*Routing.*ticket type default model/)
    assert.match(html, /class="save-banner save-dock"/)
  ...
  test('dispatch carries the switch and each editable number', () => {
    ...
    assert.match(t, /messages_per_send How many typed messages one composite send may contain/)
    assert.match(t, /live_pane_cap How many overseer conversations keep a live pane/)
  ...
  test('Connections creates the GitHub App and names each owner installation', () => {
    ...
    page.UI.set.section = 'connections'
  ...
  test('maintenance reads last, and says the daemon runs the files when it does', () => {
    ...
    assert.deepEqual(tabs, ['routing', 'projects', 'dispatch', 'connections', 'maintenance'], 'the fifth section, and it reads last')
  })
