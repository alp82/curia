...
// The GitHub card's watch choice (#891): a repository is `owner/name`, the
// shape the operator configuration contract accepts, and nothing else crosses.
const WATCH_REPO_RE = /^[\w.-]+\/[\w.-]+$/
// The Tailscale card (#877) has no field since #891: the login is never one,
// because the daemon records the identity Serve stamped on the request and
// nothing a browser typed, and the node's name is chosen at installation.
...
export class DashboardSurface {
  ...
  #handle(req, res) {
    ...
    if (req.method === 'POST') {
      ...
      // The GitHub card's watch choice (#891). The ticked repositories are
      // the whole watch list from here on, landed through the same validated
      // settings save the Settings screen uses (the daemon's `POST /settings`
      // under a root, the mounted file in the source deployment) and applied
      // through the same reload. A repository already on the list keeps its
      // mode; a new one takes `auto`. The watch-removal guard runs as it does
      // for a settings save, so a repository an agent runs on cannot be
      // unticked out from under it.
      if (url.pathname === '/api/setup/github/watch') {
        return this.#write(res, async () => {
          const b = await this.#body(req)
          if (!Array.isArray(b.repos)) throw refuse('`repos` must be a list of `owner/name` repositories')
          const repos = [...new Set(b.repos.map((r) => field(r, WATCH_REPO_RE, 'a repository written as owner/name')))]
          if (!repos.length) throw refuse('Tick at least one repository for Curia to watch.')
          const current = (await this.#readSettings()).watch ?? []
          const modes = new Map(current.map((w) => [String(w?.repo ?? ''), w?.mode ?? 'auto']))
          const patch = { watch: repos.map((repo) => ({ repo, mode: modes.get(repo) ?? 'auto' })) }
          const by = String(req.headers[LOGIN_HEADER] ?? '') || 'dashboard'
          await this.#guardWatchRemoval(patch)
          const out = await this.#saveSettings(patch)
          const reload = out.written.length ? await this.#reload(by, { written: out.written }) : null
          this.log(out.written.length
            ? `dashboard: the GitHub card set the watch list to ${repos.join(', ')} — ${reloadLine(reload)}`
            : 'dashboard: the GitHub card\'s watch choice changed nothing')
          return { ok: true, written: out.written, reload }
        })
      }
      ...
      // the identity it arrived with, and the node's name is the daemon's to
      // read (#891). The allowlist is read back from the daemon at once, so
      // the next request is admitted by the record and not by the window.
      if (url.pathname === '/api/setup/tailscale/operator') {
        return this.#write(res, async () => {
          ...
          const out = await this.#daemon({ method: 'POST', path: '/setup/tailscale/operator', body: { login }, accept: [200, 400], timeout: SETUP_TIMEOUT_MS })
          if (out.ok === false) throw refuse(out.error)
          await this.refreshIdentity()
          return out
        })
