...
//           switched opus off, and the override then refused to load.
//
...
export function routingReadiness(routing, credentialed, { provider = null } = {}) {
  ...
  for (const [builder, model] of Object.entries(routing?.review ?? {})) {
    if (model !== null && !runnable(routing, model, has)) missing.push(`review.${builder}`)
  }
...
function reviewPreset(routing, base, { provider, credentialed }) {
  ...
  const review = {}
  for (const [builder, model] of Object.entries(base?.review ?? {})) {
    if (typeof model !== 'string') continue
    if (active(model)) review[builder] = model
    else if (target && providerOfModel(routing, target) !== builder && active(target)) review[builder] = target
    else review[builder] = null
  }
  return review
}
