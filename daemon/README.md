...
**The repository picker uses App installations.** It lists the repositories each installation grants through `TokenMinter.reposFor()`, so a packaged installation needs no GitHub CLI login. **Refresh repositories** bypasses the success cache. Transient failures retry twice and retain the last successful list. **Manage repository access** opens GitHub's installation settings, and returning to Projects refreshes the list. A connection failure links to GitHub Setup. Only source deployments without an App retain the host-login picker. Gate approvals still use the operator's identity, as described next.
...
**What the host login still holds on the source daemon**: dev sessions, the deploy sibling, and the gate approval. An app cannot approve for a human, and an app-minted approval on an app-authored pull request is a self-approval again. A packaged installation has no host login, and there the approval runs on the operator's own authorization (#891, [ADR-0031](../docs/adr/0031-the-operator-authorizes-curia-when-the-app-is-installed.md)).

## The gate approval, and branch protection (#391)

The ✅ press submits a real GitHub approval. `approvePullRequest` in `github.mjs` runs `gh pr review --approve` on the pull request the gate showed with `{ operator: true }`, never with the bot's token: the approval is a person's judgement, an app cannot post one for them, and the pull request is the bot's own since #390.

**Whose account that is depends on the deployment** (#891, [ADR-0031](../docs/adr/0031-the-operator-authorizes-curia-when-the-app-is-installed.md)). On the source box no operator source is wired, and the child inherits the operator's own `~/.config/gh`, the host login, as it has since #391. Under an installation root there is no host login, and `src/githuboperator.mjs` is the source: the App's manifest asks GitHub to authorize the App as the operator when they install it (`request_oauth_on_install`, `setup_on_update`, `callback_urls`), the `/github-app/authorize` route exchanges the one-use code with the App's client id and secret (stored beside the key in `secrets/github-app.json`), reads the login on `GET /user`, and writes the token, its refresh token, their expiry, and the login to `secrets/github-operator.json`. `token()` refreshes inside a ten-minute margin, once for concurrent callers, and `daemongh.mjs` puts the value into the one approval child as `GH_TOKEN`. A root with no authorization refuses the approval naming the reinstall, and the GitHub card's verification proves the token on `GET /user` and reports the login, failing at the install step with the same cure when it is missing or refused. `test/githuboperator.test.mjs` pins the exchange, the refresh, and the refusals; `test/daemongh.test.mjs` pins the routing.
...
**The five long-lived credentials are files in `secrets/`, read through `cli/src/secrets.mjs`.** `secrets/discord-bot-token`, `secrets/github-app.json` (`{ id, pem, client_id, client_secret }`), `secrets/github-operator.json` (the operator's own authorization, `src/githuboperator.mjs`), `secrets/anthropic.json` (the store `AnthropicCredentialStore` owns), and `secrets/codex-auth.json` (the store `CodexCredentialBroker` refreshes). The Discord facts beside the token, `allowed_users`, `guild_id`, and `channel`, are `state/discord.json`, read through `src/discordsettings.mjs`. The allowed Tailscale operator, the machine name, and Curia's Serve routes are `state/tailscale.json`, read through `src/tailscalesetup.mjs`. Under a root the daemon loads no env file, and a credential key found in its environment refuses the boot by name. `GET /overview` carries `secrets`, each file by presence and never by value.
