...
- `GET /setup/discord`, `POST /setup/discord/token {token, user_id}`, and `POST /setup/discord/channel {guild_id, channel}` — the Discord card (#876, `src/discordsetup.mjs`). The read is the panel's own: the token by presence, the bot, the servers it is in, the invite link, and the facts in `state/discord.json`. The token submission lands the token in `secrets/discord-bot-token` through `writeSecret` and the operator ID in `state/discord.json`, and answers the same read. The channel choice lands the server and the channel name, reuses a top-level text channel of that name or creates one (the one write that creates anything in Discord, #891), and answers the freshly verified card. No answer, log line, or refusal carries the token; a paste that is not a token is refused by shape.
