...
- **The write (#265).** `config/` is the sidecar's only read-write mount, and the settings screen is the only thing that writes it. `POST /api/settings` edits `curia.yaml` and `routing.yaml` through the yaml **document** API, so every hand comment survives; validates the candidate with `loadCuriaConfig`/`loadRoutingConfig`, the same functions that decide whether the daemon boots; and renames it over the real file only after every candidate passes. A refused save answers 409 with the loader's own message and leaves both files byte for byte as they were. `checkPaths: false` is the one rule turned off: four loader checks ask about paths this container does not mount, and no key the screen writes can reach them. `POST /api/restart` orders the daemon's own `POST /restart`.
...
- **The poll.** `dashboard.poll_interval_s` (5) is a ceiling, not a clock. The sidecar re-reads only when a page asks and its snapshot is older than that, so many tabs cost one read and a hidden tab — which stops asking — costs none. The number matters because one read reads the whole journal off disk, and the journal grows without bound.
